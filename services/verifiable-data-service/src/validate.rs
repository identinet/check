...
fn get_jws_payload(jws_string: String) -> Result<String, FromUtf8Error> {
...
impl ClaimsDecoder<serde_json::Value> for VerifyingClaimsDecoder {
    ...
    fn decode<'a>(
        ...
        match format {
            ...
            //     //     .expect("unable to decode JWS payload");
            //     let vp_payload = get_jws_payload(vp_token).expect("unable to decode JWS payload");
            //     let vp_payload_json: serde_json::Value =
            ...
            ClaimFormatDesignation::JwtVpJson | ClaimFormatDesignation::JwtVcJson => {
                ...
                let payload = get_jws_payload(jws).expect("unable to decode JWS payload");
                let payload_json: serde_json::Value =
                    serde_json::from_str(&payload).expect("unable to deserialize payload");
                println!("Format {}: {}", format, payload_json);
